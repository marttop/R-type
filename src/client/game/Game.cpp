...
void Game::sendInput(asio::ip::udp::socket &writeUdpSocket)
{
    if (_direction[RIGHT])
        writeUdpSocket.send(asio::buffer("008 RIGHT\n"));
    if (_direction[UP])
        writeUdpSocket.send(asio::buffer("008 UP\n"));
    if (_direction[LEFT])
        writeUdpSocket.send(asio::buffer("008 LEFT\n"));
    if (_direction[DOWN])
        writeUdpSocket.send(asio::buffer("008 DOWN\n"));
    if (_shoot)
        writeUdpSocket.send(asio::buffer("008 SPACE\n"));
}

void Game::event(const sf::Event &event, const sf::RenderWindow &window)
{
    if (!_alert.isOpen()) {
...
void Game::update(const sf::RenderWindow &window, asio::ip::udp::socket &udpWriteSocket)
{
    std::vector<std::string> cmdUdp = SEPParsor::parseCommands(_udpBuf);
    openAlert();
    if (!_alert.isOpen()) {
        udpUpdateEntity(cmdUdp, window);
        sendInput(udpWriteSocket);
